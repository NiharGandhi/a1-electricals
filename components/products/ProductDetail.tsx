...
export function ProductDetail({ product, category, related }: Props) {
      ...
      ? [
          {
            ...
            drawingImages:
              product.specificationDrawingImages ??
  ...
  const safeSpecIndex = Math.min(activeSpecIndex, Math.max(specificationSections.length - 1, 0));
  const activeSpecSection = specificationSections[safeSpecIndex];
      ...
      <div className="pt-24 md:pt-28 border-b border-[var(--border)]">
        <Container>
          <div className="py-3.5 flex items-center gap-2 text-xs text-[var(--muted)] font-mono">
            ...
            <Link href={`/products/${product.categorySlug}`} className="hover:text-[var(--accent)] transition-colors">
              {category?.title}
            </Link>
            <span className="opacity-30">/</span>
        ...
        <section className="py-14 md:py-20 border-b border-[var(--border)]">
          <Container>
            <div className="flex items-end justify-between pb-7 border-b border-[var(--border)] mb-0">
              ...
              <Link href={`/products/${product.categorySlug}`} className="text-xs font-bold uppercase tracking-wider text-[var(--accent)] hover:underline flex items-center gap-1">
                View all
  ...
  return (
    <section className="pt-32 pb-20">
      <Container>
        ...
        <h1 className="display text-[var(--foreground)]" style={{ fontSize: "clamp(2rem,5vw,3rem)" }}>
          Product Not Found
        </h1>
